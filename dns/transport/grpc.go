...
import (
	...
	dnsendpoint "github.com/doublemo/balala/dns/endpoint"
...
func NewGRPCServer(endpoints dnsendpoint.Set, otTracer stdopentracing.Tracer, zipkinTracer *stdzipkin.Tracer, logger log.Logger) pb.InternalServer {
	...
	return &GRPCServer{
		call: grpctransport.NewServer(
			...
			decodeGRPCRequest,
			encodeGRPCResponse,
			append(options, grpctransport.ServerBefore(jwt.GRPCToContext(), opentracing.GRPCToContext(otTracer, "Call", logger)))...,
		),
...
func MakeFactoryCall(logger log.Logger, otTracer stdopentracing.Tracer, zipkinTracer *stdzipkin.Tracer, jwtToken []byte) sd.Factory {
	return func(instance string) (endpoint.Endpoint, io.Closer, error) {
		...
		conn, err := grpc.Dial(value.IP+":"+value.Port, grpc.WithInsecure())
		if err != nil {
			return nil, nil, err
		}

		s := NewGRPCClient(conn, otTracer, zipkinTracer, jwtToken, logger)
		doEndpoint := dnsendpoint.MakeCallEndpoint(s)
